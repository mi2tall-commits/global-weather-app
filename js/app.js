...
class WeatherApp {
  ...
  bindEvents() {
    ...
    this.elements.interval1hBtn.addEventListener('click', () => this.setInterval(1));
    this.elements.interval3hBtn.addEventListener('click', () => this.setInterval(3));
    this.elements.favBtn.addEventListener('click', () => this.toggleFavorite());

    // Gemini AI Events
    this.elements.geminiSettingsBtn.addEventListener('click', () => this.openGeminiModal());
    this.elements.geminiCloseModalBtn.addEventListener('click', () => this.closeGeminiModal());
    this.elements.geminiSaveKeyBtn.addEventListener('click', () => this.saveGeminiKey());
    this.elements.geminiBriefingBtn.addEventListener('click', () => this.triggerGeminiBriefing());
    this.elements.geminiChatSendBtn.addEventListener('click', () => this.sendGeminiChat());
  ...
  renderCurrentWeather() {
    ...
    this.elements.currentTemp.textContent = `${cur.temp}°`;
    this.elements.currentDesc.textContent = cur.weatherDesc;
    this.elements.currentIcon.textContent = cur.weatherIcon;
    this.elements.currentApparent.textContent = `${cur.apparentTemp}°C`;
    this.elements.currentHumidity.textContent = `${cur.humidity}%`;
    this.elements.currentWind.textContent = `${cur.windSpeed} km/h (순간 ${cur.windGusts})`;
    this.elements.currentPrecip.textContent = `${cur.precip} mm`;
    this.elements.currentPressure.textContent = `${cur.pressure} hPa`;
  ...
  renderConsensusAndAlerts() {
    ...
    this.elements.consensusText.textContent = insight.summaryText;
  ...
  setInterval(interval) {
    ...
    if (interval === 1) {
      this.elements.interval1hBtn.className = 'px-3 py-1.5 rounded-lg text-xs font-semibold bg-indigo-600 text-white shadow';
      this.elements.interval3hBtn.className = 'px-3 py-1.5 rounded-lg text-xs font-medium text-slate-400 hover:text-white';
    } else {
      this.elements.interval3hBtn.className = 'px-3 py-1.5 rounded-lg text-xs font-semibold bg-indigo-600 text-white shadow';
      this.elements.interval1hBtn.className = 'px-3 py-1.5 rounded-lg text-xs font-medium text-slate-400 hover:text-white';
    }
    this.renderHourlyTimeline();
  }
  ...
  updateFavButtonState() {
    ...
    this.elements.favBtn.className = isFav
      ? 'px-3 py-1.5 rounded-lg text-xs font-semibold bg-amber-500/20 text-amber-300 border border-amber-500/30'
      : 'px-3 py-1.5 rounded-lg text-xs font-medium bg-white/5 hover:bg-white/10 text-slate-300 border border-white/10';
  }

  // --- Gemini AI Logic ---
  openGeminiModal() {
    this.elements.geminiApiKeyInput.value = GeminiAI.getApiKey();
  ...
  async triggerGeminiBriefing() {
    ...
    try {
      ...
      `;
      this.elements.geminiBriefingCard.classList.remove('hidden');
      ...
      this.elements.geminiBriefingContent.innerHTML = formatted;
    } catch (err) {
      this.elements.geminiBriefingContent.innerHTML = `
